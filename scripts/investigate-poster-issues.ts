...
async function investigatePosterIssues() {
  console.log('\n' + '='.repeat(80));
  console.log('🔍 INVESTIGATING POSTER & DUPLICATE ISSUES');
  console.log('='.repeat(80) + '\n');

  // Issue 1: Kirayi/Kirai Dada duplicate
  console.log('1️⃣  CHECKING KIRAYI/KIRAI DADA:\n');
  ...
  console.log(`   Found ${kirayiMovies?.length || 0} movies:\n`);
  ...
  if (shantiKranti) {
    ...
    // Check if URL suggests wrong version
    if (shantiKranti.poster_url) {
      const url = shantiKranti.poster_url.toLowerCase();
      if (url.includes('tamil') || url.includes('ta-in') || url.includes('/ta/')) {
        console.log(`   ⚠️  WARNING: Poster URL contains Tamil indicators!`);
      }
    }
    
    console.log(`   📝 Action needed: Find Telugu poster with Nagarjuna\n`);
  } else {
    console.log('   ❌ Movie not found!\n');
  }
