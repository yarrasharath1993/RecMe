...
async function applyAttribution(attr: Attribution, dryRun: boolean): Promise<boolean> {
  ...
  const { data: movie } = await supabase
    .from('movies')
    .select('id, title_en, cast_members, supporting_cast, hero, heroine, director, directors, producer, producers, music_director, cinematographer, writer, writers, crew')
    .eq('id', attr.movieId)
    .single();
...
async function main() {
  ...
  const csvPath = path.join(process.cwd(), 'MISSING-MOVIES-VERIFICATION.csv');
