...
import { parseCastMembers } from "@/lib/utils/cast-parser";
import { isValidImageUrl } from "@/lib/utils/safe-image";
import { MoviePlaceholder } from "@/components/movies/MoviePlaceholder";
...
export async function generateMetadata({
  ...
  const { data: movie } = await supabase
    .from("movies")
    .select("title_en, title_te, synopsis, poster_url, director, avg_rating, release_year, release_date, slug")
    .eq("slug", slug)
    .single();

  if (!movie) return { title: "Movie Not Found" };

  // Check if movie is upcoming (don't show rating in metadata)
...
async function getMovieData(slug: string) {
  ...
  // Includes director, hero, heroine, support cast, music director, producer, genre, era sections
  // Target: 150-200 movies across multiple sections
  const similarSections = await getSimilarMovieSections(
    ...
    undefined, // allMovies - let it fetch from database
    { 
      maxSections: 15,        // Allow up to 15 sections for more variety
      maxMoviesPerSection: 100 // 100 movies per section (Discover More: min 50, max 100)
    }
  );
    ...
    <main className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <div className="lg:hidden">
        ...
        <section className="relative">
          ...
          <div className="absolute inset-0 h-[200px]">
            {movie.poster_url && isValidImageUrl(movie.poster_url) && (
              <Image
          ...
          <div className="relative px-4 pt-4 pb-3">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                {/* Coming Soon Badge or Rating Badge */}
                {isUpcoming ? (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-orange-500/20 to-red-500/20 border border-orange-500/40 rounded-lg mb-2 w-fit">
                    <Calendar className="w-4 h-4 text-orange-400" />
                    <span className="text-orange-300 font-bold text-sm">COMING SOON</span>
                    {movie.release_year && <span className="text-orange-200/80 text-xs">• {movie.release_year}</span>}
                  </div>
                ...
                <div className="flex gap-1 mt-1.5 items-center flex-wrap">
                  ...
                  {/* Verified Badge */}
                  {movie.data_confidence !== undefined && movie.data_confidence >= 0.6 && (
                    <span className="h-5 px-2 rounded text-[10px] font-medium whitespace-nowrap bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-500/30 inline-flex items-center gap-1">
        ...
        <section className="px-4 py-2 border-t border-[var(--border-primary)]/30 relative z-10 bg-[var(--bg-primary)]">
          ...
          >
            {parseCastMembers(movie)
              .slice(0, 6)
              .map((cast, i) => {
                // Shorten role labels for mobile
                const shortRole = cast.role
                  .replace("Director", "Dir")
                  .replace("Heroine", "Her")
                  .replace("Producer", "Prod");
                return (
                  <span
                    key={i}
      ...
      <section className="hidden lg:block relative">
        ...
        <div className="absolute inset-0 h-[500px]">
          {movie.backdrop_url && isValidImageUrl(movie.backdrop_url) ? (
            <Image
        ...
        <div className="relative max-w-7xl mx-auto px-4 pt-6 pb-8">
          <div className="flex gap-6">
            ...
            {/* Middle: Info */}
            <div className="flex-1 min-w-0">
              {/* Title Row with Rating or Coming Soon */}
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                {isUpcoming ? (
                  <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500/20 to-red-500/20 border border-orange-500/40 rounded-lg">
                    <Calendar className="w-5 h-5 text-orange-400" />
                    <div>
                      <span className="text-orange-300 font-bold text-base block">COMING SOON</span>
                      {movie.release_year && <span className="text-orange-200/80 text-xs">{movie.release_year}</span>}
                    </div>
                  </div>
              ...
              <div className="w-80 flex-shrink-0">
                <div className="sticky top-4 space-y-4">
                  <QuickVerdictCard
                    ...
                    contentWarnings={movie.trigger_warnings}
                  />
                </div>
              </div>
            )}
      ...
      {/* Movie Impact Analysis - Phase 1: Top Movies Only */}
